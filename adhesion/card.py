...
def generate_card(adhesion):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="' + str(adhesion))

    # Create the background
    c = canvas.Canvas(response)
    if adhesion.adhesion_type == "Adhésion AEIR":
        c.drawImage(
            "static/card/AEIR_card_model.png", 0, 0, width=595.27, height=841.89
        )
    elif adhesion.adhesion_type == "Adhésion Foyer":
        c.drawImage("static/card/FOY_card_model.png", 0, 0, width=595.27, height=841.89)
    ...
    # Display picture
    c.drawImage(adhesion.picture.path, 314, 365, height=292, width=230)
